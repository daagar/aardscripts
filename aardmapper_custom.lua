...
local drag_initial_offset_x, drag_initial_offset_y
local drag_current_offset_x, drag_current_offset_y
local drag_save_timer_id = "mapper_drag_save_timer"
local drag_last_move_time = 0
local save_offset
...
local function get_room (uid)
   ...
   if uid == drag_room_uid then
      room.offset_x = drag_initial_offset_x + drag_current_offset_x
      room.offset_y = drag_initial_offset_y + drag_current_offset_y
   end
...
local function draw_room (uid, path, x, y, keep_hotspots)
   ...
   local offset_x = room.offset_x or 0
   local offset_y = room.offset_y or 0
   
   if uid == tostring(drag_room_uid) then
      offset_x = drag_initial_offset_x + drag_current_offset_x
      offset_y = drag_initial_offset_y + drag_current_offset_y
   end

   local display_x = x + offset_x * grid_size
   local display_y = y + offset_y * grid_size

   local coords = string.format ("%i,%i", math.floor (display_x), math.floor (display_y))
   ...
   drawn [uid] = { coords = coords, path = path, x = x, y = y }

   local left, top, right, bottom = display_x - HALF_ROOM, display_y - HALF_ROOM, display_x + HALF_ROOM, display_y + HALF_ROOM

   -- forget it if off screen
   if (display_x < HALF_ROOM) or (display_y < (title_bottom or font_height)+HALF_ROOM) or
      (display_x > config.WINDOW.width - HALF_ROOM) or (display_y > config.WINDOW.height - HALF_ROOM) then
      return
   end -- if
   ...
   for dir, exit_uid in pairs (room.exits) do
      ...
      if exit_info then
         ...
         exit_uid = tostring(exit_uid)
         local exit_room = rooms [exit_uid] or get_room (exit_uid)
         rooms[exit_uid] = exit_room

         local next_x = x + exit_info.at [1] * grid_size
         local next_y = y + exit_info.at [2] * grid_size
         
         local ex_offset_x = exit_room.offset_x or 0
         local ex_offset_y = exit_room.offset_y or 0
         if exit_uid == tostring(drag_room_uid) then
            ex_offset_x = drag_initial_offset_x + drag_current_offset_x
            ex_offset_y = drag_initial_offset_y + drag_current_offset_y
         end

         local next_display_x = next_x + ex_offset_x * grid_size
         local next_display_y = next_y + ex_offset_y * grid_size

         local next_coords = string.format ("%i,%i", math.floor (next_display_x), math.floor (next_display_y))
         ...
         if config.SHOW_AREA_EXITS and room.area ~= rooms [exit_uid].area and not rooms[exit_uid].unknown then
            area_exits [ rooms [exit_uid].area ] = area_exits [ rooms [exit_uid].area ] or {x = display_x, y = display_y, def = barriers[dir]}
         end -- if
         ...
         local source_x = display_x + exit_info.x1
         local source_y = display_y + exit_info.y1
         local dest_x, dest_y
         
         if exit_info == stub_exit_info or dir == "u" or dir == "d" then
            dest_x = display_x + exit_info.x2
            dest_y = display_y + exit_info.y2
         else
            dest_x = next_display_x - exit_info.at[1] * HALF_ROOM
            dest_y = next_display_y - exit_info.at[2] * HALF_ROOM
         end
 
         WindowLine (win, source_x, source_y, dest_x, dest_y, exit_line_colour, linetype + 0x0200, linewidth)
...
function init (t)
   ...
   save_offset = t.save_offset            -- callback to save offset
...
function invalidate_room_cache(uid)
   uid = tostring(uid)
   rooms[uid] = nil
end -- invalidate_room_cache
...
function mousedown_room (flags, hotspot_id)
   if bit.band (flags, miniwin.hotspot_got_lh_mouse) ~= 0 and bit.band (flags, miniwin.hotspot_got_shift) ~= 0 then
      drag_room_uid = tostring(hotspot_id)
      drag_start_x = WindowInfo (win, 14)
      drag_start_y = WindowInfo (win, 15)
      local room = rooms [drag_room_uid]
      if not room then
         room = get_room (drag_room_uid)
      end
      drag_initial_offset_x = room.offset_x or 0
      drag_initial_offset_y = room.offset_y or 0
      drag_current_offset_x = 0
      drag_current_offset_y = 0
   end
end

function dragmove_room (flags, hotspot_id)
   if not drag_room_uid then 
      return 
   end
   local cur_x = WindowInfo (win, 14)
   local cur_y = WindowInfo (win, 15)
   local delta_x = cur_x - drag_start_x
   local delta_y = cur_y - drag_start_y
   
   local grid_size = ROOM_SIZE + DISTANCE_TO_NEXT_ROOM
   local off_x = math.floor (delta_x / grid_size + 0.5)
   local off_y = math.floor (delta_y / grid_size + 0.5)
...
function dragrelease_room (flags, hotspot_id)
   Note("[DRAG] dragrelease_room called, drag_room_uid=" .. tostring(drag_room_uid))
   if not drag_room_uid then return end
   local final_x = drag_initial_offset_x + drag_current_offset_x
   local final_y = drag_initial_offset_y + drag_current_offset_y
   Note(string.format("[DRAG] Room %s: initial=(%d,%d) current=(%d,%d) final=(%d,%d)", 
      drag_room_uid, drag_initial_offset_x, drag_initial_offset_y, 
      drag_current_offset_x, drag_current_offset_y, final_x, final_y))
   if final_x ~= drag_initial_offset_x or final_y ~= drag_initial_offset_y then
       Note(string.format("[DRAG] Offset changed, calling save_offset. Type: %s", type(save_offset)))
       -- Call back to the plugin to save the offset
       if type (save_offset) == "function" then
          save_offset (drag_room_uid, final_x, final_y)
       else
          Note("[DRAG] ERROR: save_offset is not a function!")
       end
   else
       Note("[DRAG] No offset change detected")
   end
   drag_room_uid = nil
   draw (current_room)
end
